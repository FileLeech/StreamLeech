...
if (!defined('rl_UserAgent'))
    define('rl_UserAgent', 'Opera/9.80 (Windows NT 6.1) Presto/2.12.388 Version/12.17');
...
function is_page($lpage) {
    global $lastError;
    if (!$lpage)
        html_error(lang(84) . "<br />$lastError");
}
...
function cURL($link, $cookie = 0, $post = 0, $referer = 0, $auth = 0, $opts = 0) {
    ...
    if (substr($page, 9, 3) == '100' || !empty($opt[CURLOPT_HTTPPROXYTUNNEL]))
...
function GetCookies($content) {
    if (($hpos = strpos($content, "\r\n\r\n")) > 0)
        $content = substr($content, 0, $hpos); // We need only the headers
...
function GetCookiesArr($content, $cookie = array(), $del = true, $dval = array('', 'deleted')) {
    ...
    foreach ($temp[1] as $v) {
        ...
        if ($del) {
            if (!is_array($dval))
                $dval = array($dval);
            if (in_array($v[1], $dval))
                unset($cookie[$v[0]]);
            elseif (!empty($p)) {
                if (stripos($p, 'Max-Age=') !== false && preg_match('/[ \;]?Max-Age=(-?\d+)/i', $p, $P) && (int) $P[1] < 1)
                    unset($cookie[$v[0]]);
                elseif (stripos($p, 'expires=') !== false && preg_match('/[ \;]?expires=([a-zA-Z]{3}, \d{1,2} [a-zA-Z]{3} \d{4} \d{1,2}:\d{1,2}:\d{1,2} GMT)/i', $p, $P) && ($P = strtotime($P[1])) !== false && $P <= time())
                    unset($cookie[$v[0]]);
            }
        }
    }
...
function upfile($host, $port, $url, $referer, $cookie, $post, $file, $filename, $fieldname, $field2name = '', $proxy = 0, $pauth = 0, $upagent = 0, $scheme = 'http') {
    ...
    if ($proxy)
        echo '<p>' . sprintf(lang(89), $proxyHost, $proxyPort) . '<br />UPLOAD: <b>' . htmlspecialchars($url) . "</b>...<br />\n";
    else
        echo '<p>' . sprintf(lang(90), $host, $port) . '</p>';
    ...
    $sFilters = array();
    if (stripos($header, "\nTransfer-Encoding: chunked") !== false && in_array('dechunk', stream_get_filters()))
        $sFilters['dechunk'] = stream_filter_append($fp, 'dechunk', STREAM_FILTER_READ); // Add built-in dechunk filter
