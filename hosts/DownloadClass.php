...
class DownloadClass {
    ...
    public function RedirectDownload($link, $FileName, $cookie = 0, $post = 0, $referer = 0, $force_name = 0, $auth = 0, $addon = array()) {
        ...
        if (!empty($force_name))
            $params['force_name'] = urlencode($force_name);
        $params['host'] = urlencode($url['host']);
        if (!empty($url['port']))
            $params['port'] = urlencode($url['port']);
        $params['path'] = urlencode($url['path'] . (!empty($url['query']) ? '?' . $url['query'] : ''));
        if (!empty($post))
            $params['post'] = urlencode(encrypt(serialize($post)));
        if (!empty($auth))
            $params['auth'] = ($auth == '1' ? '1' : urlencode(encrypt(base64_encode($auth))));
        if (!empty($addon)) {
            if (!is_array($addon))
                html_error('Plugin problem! Please report, error: "The parameter passed must be an array"'); // Some problems with the plugin, quit it
            foreach ($addon as $name => $value)
                $params[$name] = (is_array($value) ? urlencode(serialize($value)) : urlencode($value));
        }
    ...
    public function moveToAutoDownloader($link_array) {
        ...
        if (empty($link_array) || !is_array($link_array) || count($link_array) == 0)
            html_error('Error getting links from folder.');

        if (!is_file('audl.php') || !empty($options['auto_download_disable']))
    ...
    public function JSCountdown($secs, $post = 0, $text = 'Waiting link timelock', $stop = 1) {
        ...
        public function reCAPTCHA($publicKey, $inputs, $referer = 0, $sname = 'Download File') {
            if (empty($publicKey) || preg_match('/[^\w\.\-]/', $publicKey))
                html_error('Invalid reCAPTCHA public key.');
            if (!is_array($inputs))
                html_error('Error parsing captcha post data.');
            ...
            if (substr($page, 9, 3) != '200')
                html_error('Invalid or deleted reCAPTCHA public key.');
            ...
            if (strpos($page, 'Invalid referer') === false && strpos($page, 'An internal error occurred') === false) {
                ...
                if (strpos($page, 'Invalid referer') !== false)
                    html_error('Error getting reCAPTCHA challenge: Bad referer');
                if (strpos($page, 'An internal error occurred') !== false)
                    html_error('Error getting reCAPTCHA challenge: Possible reCAPTCHA v2 or bad referer');

                if (!preg_match('@[\{,\s]challenge\s*:\s*[\'\"]([\w\.\-]+)[\'\"]@', $page, $challenge))
                    html_error('Error getting reCAPTCHA challenge.');
        ...
        public function SolveMedia($skey, $data, $referer = 0, $sname = 'Download File') {
            if (!is_array($data))
                html_error('Post needs to be sended in a array.');
            if (empty($skey) || preg_match('@[^\w\-\.]@', $skey))
                html_error('Invalid value for $skey');
            $page = $this->GetPage("http://api.solvemedia.com/papi/challenge.noscript?k=$skey", 0, 0, $referer);
            if (!preg_match('@<img [^/<>]*src\s?=\s?\"((https?://[^/\"<>]+)?/papi/media[^\"<>]+)\"@i', $page, $imgurl))
                html_error('[SM] CAPTCHA img not found.');
            $imgurl = (empty($imgurl[2])) ? 'http://api.solvemedia.com' . $imgurl[1] : $imgurl[1];

            if (!preg_match_all('@<input [^/|<|>]*type\s?=\s?\"?hidden\"?[^/<>]*\s?name\s?=\s?\"(\w+)\"[^/<>]*\s?value\s?=\s?\"([^\"<>]+)\"[^/<>]*/?\s*>@i', $page, $forms))
                html_error('[SM] CAPTCHA data not found.');
            $forms = array_combine($forms[1], $forms[2]);
            foreach ($forms as $n => $v)
                $data["_smc[$n]"] = urlencode($v);
            $data['sm_public_key'] = $skey; // Required for verifySolveMedia()
            //Download captcha img.
            list($headers, $imgBody) = explode("\r\n\r\n", $this->GetPage($imgurl), 2);
            if (substr($headers, 9, 3) != '200')
                html_error('[SM] Error downloading CAPTCHA img.');
        ...
        public function verifySolveMedia($directOutput = false) {
            if (empty($_POST['adcopy_response']))
                html_error('[SM] You didn\'t enter the image verification code.');
            if (empty($_POST['_smc']) || !is_array($_POST['_smc']))
                html_error('[SM] CAPTCHA data invalid.');
            $post = array();
            foreach ($_POST['_smc'] as $n => $v)
                $post[urlencode($n)] = $v;
            ...
            if (!preg_match('@>\s*([^<>\s]+)\s*</textarea>@i', $page, $gibberish))
                html_error('[SM] CAPTCHA response not found.');

            if ($directOutput)
                return urlencode($gibberish[1]);
            else
        ...
        public function reCAPTCHAv2($publicKey, $inputs, $referer = 0, $sname = 'Download File') {
            if (empty($publicKey) || preg_match('/[^\w\.\-]/', $publicKey))
                html_error('Invalid reCAPTCHA2 public key.');
            if (!is_array($inputs))
                html_error('[RC2] Error parsing captcha post data.');

            // Check key
            $page = $this->GetPage('http://www.google.com/recaptcha/api/fallback?k=' . urlencode($publicKey), array('PREF' => 'LD=en'), 0, $referer);
            if (substr($page, 9, 3) != '200')
                html_error('Invalid or deleted reCAPTCHA2 public key.');
            $inputs['recaptcha2_public_key'] = $publicKey; // Required for validateReCaptchav2()
            // Download captcha
            if (!preg_match('@name="c" value="([\w\.\-]+)\"@', $page, $challenge))
                html_error('Error getting reCAPTCHA2 challenge.');
            ...
            if (substr($headers, 9, 3) != '200')
                html_error('[RC2] Error downloading captcha img.');
            $mimetype = (preg_match('@image/[\w+]+@', $headers, $mimetype) ? $mimetype[0] : 'image/jpeg');
        ...
        public function verifyReCaptchav2($directOutput = false) {
            if (empty($_POST['recaptcha2_response_field']))
                html_error('[RC2] You didn\'t enter the image verification code.');
            if (empty($_POST['recaptcha2_challenge_field']) || empty($_POST['recaptcha2_public_key']) || preg_match('/[^\w\.\-]/', $_POST['recaptcha2_public_key']))
                html_error('Invalid reCaptcha2 data.');
            ...
            if (stripos($page, 'Copy this code') === false)
                html_error('[RC2] Unknown error after sending captcha.');

            if (!preg_match('@>\s*([^<>\s]+)\s*</textarea>@i', $page, $gibberish))
                html_error('[RC2] Validated response not found.');
