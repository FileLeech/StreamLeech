...
while (false !== ($entry = $d->read())) {
    if (strtolower(strrchr($entry, '.')) == '.php' && stripos($entry, '.JD') === false) {
        ...
        if (array_key_exists($hostname, $HostnamesToFix))
            $hostname = $HostnamesToFix[$hostname];
        if (in_array($hostname, $HostnamesToIgnore, true))
            continue;
        $host[$hostname] = $entry;
        switch ($hostname) {
            //define the domain so we dont need to make new plugin with only 3-6 lines code
            case '1fichier.com':
                foreach (array('alterupload.com', 'cjoint.net', 'desfichiers.com', 'dfichiers.com', 'megadl.fr', 'mesfichiers.org', 'piecejointe.net', 'pjointe.com', 'tenvoi.com', 'dl4free.com') as $d1fichier)
            ...
            case 'depositfiles.com':
                foreach (array('depositfiles.in', 'depositfiles.mobi', 'depositfiles.net', 'depositfiles.net.cn', 'depositfiles.org', 'depositfiles.co.uk', 'depositfiles.info', 'dfiles.eu', 'dfiles.ru', 'dfiles.co', 'dfiles.co.uk') as $dfdomains)
                    $host["$dfdomains"] = $host['depositfiles.com'];
                break;
            case 'd-h.st':
            ...
            case 'freakshare.com':
                ...
                break;
            case 'keep2share.cc':
                foreach (array('keep2share.com', 'keep2s.cc', 'k2s.cc') as $k2sdomains)
                    $host["$k2sdomains"] = $host['keep2share.cc'];
                break;
            case 'mega.co.nz':
            ...
            case 'turbobit.net':
                ...
                break;
            case 'uploaded.net':
                foreach (array('ul.to', 'uploaded.to') as $uploaded)
                    $host["$uploaded"] = $host['uploaded.net'];
                break;
            case 'uploadhero.co':
