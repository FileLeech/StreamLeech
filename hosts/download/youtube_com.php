...
class youtube_com extends DownloadClass {
    ...
    private function getFmtMaps() {
        ...
        if (!empty($this->response['reason']))
            html_error('[' . htmlspecialchars($this->response['errorcode']) . '] ' . htmlspecialchars($this->response['reason']));

        if (isset($_REQUEST['step']) || substr($this->page, 9, 3) == '402' || preg_match('@Location: https?://(www\.)?youtube\.com/das_captcha@i', $this->page))
            $this->captcha();

        if (empty($this->response['url_encoded_fmt_stream_map']))
            html_error('[' . (!empty($this->sts) ? htmlspecialchars($this->sts) : 0) . '] Video links not found.');
    ...
    private function findFunction($fName, $num) {
        ...
        if (count($obj) > 1) {
            ...
            if (empty($this->jsVars[$obj]['fn'][$fName]['step'])) {
                ...
                elseif (preg_match("@var\s+($v)=($v)\[0\];\\2\[0\]=\\2\[{$src[1]}%\\2(?:\.length|\[$v\])\];\\2\[{$src[1]}\]=\\1@", $src[2])) {
                    ...
                    return "s$num";
                } elseif (preg_match("@(?:$v=)?$v(?:\.reverse|\[$v\])\(\)@", $src[2]))
                    return $this->jsVars[$obj]['fn'][$fName]['step'] = 'r';
                else
                    $this->decError("Error parsing function '$obj.$fName'");
            } else
        ...
        if (empty($this->jsVars[$fName]['step'])) {
            if (($spos = strpos($this->playerJs, "function $fName(")) === false || ($epos = strpos($this->playerJs, '};', $spos)) === false)
                $this->decError("Cannot find function '$fName'");
            ...
            if (!preg_match("@^function\s+$fName\($v(?:,($v))?\)\{(.*)$@", $this->jsVars[$fName]['src'], $pars))
                $this->decError("Cannot parse function '$fName'");
            if (empty($pars[1]))
            ...
            elseif (preg_match("@var\s+($v)=($v)\[0\];\\2\[0\]=\\2\[{$pars[1]}%\\2(?:\.length|\[$v\])\];\\2\[{$pars[1]}\]=\\1@", $src[2])) {
                ...
                $this->decError("Error parsing function '$fName'");
        } else
    ...
    private function getCipher() {
        ...
        if (!preg_match('@"sts"\s*:\s*(\d+)@i', $page, $this->sts))
            html_error('Signature timestamp not found.');
        $this->sts = $this->sts[1];

        $savefile = DOWNLOAD_DIR . 'YT_lastjs.txt';
        if (!preg_match('@html5player-([\w\-\.]+(?:/\w+)?)\.js@i', str_replace('\\/', '/', $page), $this->js))
            html_error('YT\'s player javascript not found.');
        if (@file_exists($savefile) && ($file = file_get_contents($savefile, NULL, NULL, -1, 822)) && ($saved = @unserialize($file)) && is_array($saved) && !empty($saved['sts']) && $saved['sts'] == $this->sts && !empty($saved['steps']) && preg_match('@^\s*([ws]\d+|r)( ([ws]\d+|r))*\s*$@', $saved['steps'])) {
            ...
            if (($spos = strpos($this->playerJs, '.sig||')) === false)
                $this->decError('Not found (".sig||")');
            if (($cut1 = cut_str(substr($this->playerJs, $spos), '{', '}')) == false)
                $this->decError('Cannot get inner content of "if(X.sig||X.s)"');
            $v = '[\$_A-Za-z][\$\w]*';
            if (!preg_match("@(?<=\.sig\|\|)$v(?=\($v\.s\))@", $cut1, $fn))
                $this->decError('Cannot get decoder function name');
            $fn = $fn[0];
            if (($fpos = strpos($this->playerJs, "function $fn(")) === false)
                $this->decError('Cannot find decoder function');
            if (($cut2 = cut_str(substr($this->playerJs, $fpos), '{', '}')) == false)
                $this->decError('Cannot get decoder function contents');
            ...
            foreach (array_map('trim', explode(';', '{' . $this->sigJs . '}')) as $step) {
                if (($step{0} == '{' || substr($step, strlen($step) - 1, 1) == '}') && (preg_match("@^\{(?:var\s+)?$v=$v(?:\.split|\[$v\])\(\"\"\)$@", $step) || preg_match("@^return\s+$v(?:\.join|\[$v\])\(\"\"\);?\}$@", $step)))
                    continue;
                elseif (preg_match("@^(?:$v=)?((?:$v.)*$v)\($v\,(\d+)\)$@", $step, $s))
                    $this->encS[] = $this->findFunction($s[1], $s[2]);
                //elseif (preg_match("@^$v=$v\($v\,(\d+)\)$@", $step, $s)) $this->encS[] = 'w'.$s[1];
                elseif (preg_match("@^(?:$v=)?$v(?:\.s(p)?lice|\[$v\])\((?(1)0,)(\d+)\)$@", $step, $s))
                    $this->encS[] = 's' . $s[2];
                elseif (preg_match("@^(?:$v=)?$v(?:\.reverse|\[$v\])\(\)$@", $step))
                    $this->encS[] = 'r';
                else
                    $this->decError($step . ' | Unknown step on decoder function.');
            }

            if (empty($this->encS))
                $this->decError('Empty decoded result');
    ...
    private function sigDecode($sig) {
        if (empty($this->encS))
            $this->decError('sigDecode() can\'t be called before getCipher()');
        $_sig = $sig;
        $sig = str_split($sig);
        foreach ($this->encS as $_step) {
            if (!preg_match('@^\s*([wrs])(\d*)\s*$@', $_step, $step) || ($step[1] != 'r' && !array_key_exists(2, $step)))
                $this->decError("Unknown decoding step \"$_step\"");
            switch ($step[1]) {
                case 'w': $step[2] = (int) $step[2];
                    $x = $sig[0];
                    $sig[0] = $sig[$step[2] % count($sig)];
                    $sig[$step[2]] = $x;
                    break;
                case 's': $step[2] = (int) $step[2];
                    $sig = array_slice($sig, $step[2]);
                    break;
                case 'r': $sig = array_reverse($sig);
                    break;
            }
        }
        return implode($sig);
    ...
    private function QSelector() {
        ...
        $sizes = array();
        $code = '';
        foreach ($this->fmturlmaps as $key => $value) {
            $calidad = @$q[$key];
            if ($calidad != '') {
                $default = '';
                if (@$calidad == 'SD') {
                ...
                if ($code == '') {
                    $code = "{file: '" . urldecode($value) . "',label: '" . $calidad . "',type:'video/mp4'" . $default . "}";
                } else {
                    $code.=",{file: '" . urldecode($value) . "',label: '" . $calidad . "',type:'video/mp4'" . $default . "}";
                }
            }
        ...
        if ($code == '') {
            html_error("no se logro obtener las url de youtube");
        } else {
            $code = "[" . $code . "]";
            $this->setJSMedia("media_source", $code);
        }
        //sort($this->fmturlmaps);
        /* print_r($this->fmturlmaps);
